...
export function CruiseDetail({ cruiseId, initialData }: CruiseDetailProps) {
  const [cruise, setCruise] = useState<any>(initialData || null)
  ...
  useEffect(() => {
    // If we already have initialData, don't fetch again
  ...
  return (
    <div>
        ...
        {images.length > 1 && (
          <div className="flex gap-2 mt-2">
            {images.map((img: string, index: number) => (
              <button
                key={index}
                className={`relative w-20 h-14 rounded-lg overflow-hidden border-2 transition-colors ${index === currentImage ? "border-primary" : "border-transparent"
                  }`}
                onClick={() => setCurrentImage(index)}
              >
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">(186 reviews)</span>
            </div>
      ...
      <div className="mb-8">
        <h3 className="font-serif text-xl font-semibold text-foreground mb-4">Trip Information</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-muted/50 rounded-xl border border-border">
            ...
            <p className="text-sm text-muted-foreground">{cruise.meetingPointEn || "Arrival Airport or Train Station or Hotel"}</p>
          </div>
          <div className="p-4 bg-muted/50 rounded-xl border border-border">
            <h4 className="font-semibold mb-2 flex items-center gap-2">
              <MapPin className="h-4 w-4 text-primary" />
            ...
            <p className="text-sm text-muted-foreground">{cruise.endingPointEn || "Departure Airport or Train Station or Hotel"}</p>
          </div>
          <div className="p-4 bg-muted/50 rounded-xl border border-border">
            <h4 className="font-semibold mb-2 flex items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">
              Check-in: {cruise.checkIn || "11:00 AM"} | Check-out: {cruise.checkOut || "08:00 AM"}
            </p>
          </div>
          <div className="p-4 bg-muted/50 rounded-xl border border-border">
            <h4 className="font-semibold mb-2 flex items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">{cruise.groupSizeEn || "Small groups, average 15-20 people"}</p>
          </div>
        </div>
      </div>
    </div>
