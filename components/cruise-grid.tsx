...
export function CruiseGrid({ cruises }: CruiseGridProps) {
  ...
  const displayCruises = cruises.map((cruise, index) => ({
    ...cruise,
    id: cruise.id,
    name: cruise.nameEn,
    ...
    rating: 4.8,
    reviews: 142 + (index * 13), // Deterministic pseudo-random reviews count
    price: cruise.price || (1299 + (index * 100)),
    originalPrice: Math.round((cruise.price || 1299) * 1.2),
    guests: "2-4",
    tag: cruise.tags?.[0] || (index === 0 ? "Best Seller" : index === 1 ? "Luxury" : "Featured"),
